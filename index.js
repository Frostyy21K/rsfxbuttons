...
console.log(chalk.cyan(`Made by aybeecee`))
console.log(chalk `{yellowBright Credits } | {blueBright aybeecee} | {cyanBright https://discord.gg/xBm2KDMRmx}`)
...
async function checktoken(token) {
  ...
  if (token.length != "MTAxNTc3NDYxNDgxMjYyNjk4NQ.G_oYi7.JmicTqwINSsUgiiSUTHTMtcQs0HvYrPH9xAAPo".length) {
...
client.on('ready', async () => {
  ...
  client.user.setActivity(config.status.name, {
    type: config.status.type.toUpperCase()
  })
});
...
> \`${prefix}setstaff\` - set staff roles`)
      .addField(`My Source Code - Doesn't exist`, false)
      .setTimestamp()
      .setColor(0x5865F2)
      .setFooter(`All rights belong to RSFX`)
    message.reply({
      embeds: [embed],
  ...
  if (command == prefix + `add`) {
    if (!message.member.permissions.has("MANAGE_GUILD")) return message.reply({
      content: `:x: This command requires \`MANAGE_MESSAGES\` permission.`
    });
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      if (!member) return message.reply({
        content: `Mention a member of its ID`
      });
      try {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      setTimeout(async () => {
        try {
          ...
          channel.send({
            embed: {
              description: `Ticket opened by <@!${message.author.id}>`,
              color: `GREEN`
            }
          })
  ...
  if (command == prefix + 'rename' || command == prefix + 'setname') {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      if (!args) return message.reply({
        embed: {
          description: `Please select the name you want for the ticket`,
          color: 0x5865F2
        }
      })
  ...
  if (command == prefix + 'setstaff') {
    ...
    if (!message.member.permissions.has("ADMINISTRATOR")) return message.reply({
      content: `:x: This command requires \`ADMINISTRATOR\` permission.`
    });
  ...
  if (command == prefix + 'setchannels') {
    ...
    let main = new Discord.MessageEmbed()
      .setTitle(`Error`)
      .setDescription(`Please mention a ctegoryid with this command! `)
    ...
    if (cat.type !== "GUILD_CATEGORY") return message.channel.send({
      content: "The input should be a category"
    });
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    if (!sfats || sfats === null) return message.reply({
      embeds: [sfatsembed]
    })
    if (!sfas || sfas === null) return message.reply({
      embeds: [channelsetup]
    })
    let idd = randomstring.generate({
      length: 20
    })
    let args = message.content.split(' ').slice(1).join(' ');
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId == `ss`) {
    ...
    interaction.guild.channels.create(getChannelName(interaction.user), {
      ...
      reason: "All rights reserved to aybeecee"
    }).then(async channel => {
      channel = channel
  ...
  **Your ticket has been successfully opened** <#${channel.id}>`, true)
  ...
  Please specify your problem in detail so our staff team can help straight away!\n\nTo **close** this ticket, interact with 🔒`)
  ...
  **Your ticket has been successfully opened** <#${channel.id}>`, true)
      ...
      const embedticket = new Discord.MessageEmbed()
        .setTimestamp()
        .setTitle("Purchase GFX")
        .setFooter(`Ticket opened at`)
        .setColor(0x5865F2)
        .setDescription(`Support will be with you soon.\nPlease note we only accept Cashapp currently!\n\n
  To **close** this ticket, interact with 🔒`)
  ...
  **Your ticket has been successfully opened** <#${channel.id}>`, true)
  ...
  if (interaction.customId == `cl`) {
    interaction.reply({
      content: `Your order is executed after 5 seconds, and it will be closed`,
      ephemeral: true
    })
